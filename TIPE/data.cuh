...
public:
    ...
    sqlite3_stmt* creer_requete(std::string requete) {
        ...
        sqlite3_stmt* stmt;
        code_ret = sqlite3_prepare_v2(BDD, requete.c_str(), (int)requete.size(), &stmt, &pzTail);
...
public:
    ...
    void fixer(long long date_heure) {
        _annee = (int)(date_heure / 100000000);
        _moi = (int)((date_heure / 1000000) % 100);
        _jour = (int)((date_heure / 10000) % 100);
        _heure = (int)((date_heure / 100) % 100);
        _minutes = (int)(date_heure % 100);
    }
    ...
    Date add(Date date) {
        struct tm tm1 = get_tm();
        time_t time = std::mktime(&tm1);
        struct tm tm2 = date.get_tm();
        time += std::mktime(&tm2);
        std::time(&time);
...
static std::string __my_to_string__(int i) {
    if (i < 10) {
...
struct VariationException : public std::exception {
    const char* what() const throw() {
        return "Exception : Ordre invalide !\n";
    }
};
template<typename T> static T get_variation(std::vector<T>& vec, int i, int ordre) {
    if (ordre == 0) {
        if (0 <= i && i < vec.size()) { return vec[i]; }
        throw VariationException();
    }
    else {
        try
        {
            return get_variation<T>(vec, i, ordre - 1) - get_variation<T>(vec, i - 1, ordre - 1);
        }
        catch (VariationException&)
        {
            return 0;
        }
    }
...
public:
    ...
    std::vector<Date> times(int deb = -1, int fin = -1) {
        ...
        if (deb == -1) { deb = 0; }
        if (fin == -1) { fin = (int)date_heure_.size(); }
    ...
    void recup_annee(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre) {
    ...
    void recup_moi(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre) {
        for (int i = 0; i < nombre; i++) {
            int j = i + deb;
            int k = i + deb_X;
    ...
    void recup_jour(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre) {
        for (int i = 0; i < nombre; i++) {
            int j = i + deb;
            int k = i + deb_X;
    ...
    void recup_heure(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre) {
        for (int i = 0; i < nombre; i++) {
            int j = i + deb;
            int k = i + deb_X;
    ...
    void recup_minutes(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre) {
        for (int i = 0; i < nombre; i++) {
            int j = i + deb;
            int k = i + deb_X;
    ...
    void recup_jour_semaine(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre) {
        for (int i = 0; i < nombre; i++) {
            int j = i + deb;
            int k = i + deb_X;
    ...
    void recup_temperature(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre, int ordre_variation = 0) {

        for (int i = 0; i < nombre; i++) {
            int j = i + deb;
            int k = i + deb_X;
            X.set(ligne_X, k, get_variation<double>(temperature_, j, ordre_variation));
        }

    }
    void recup_humidite(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre, int ordre_variation = 0) {

        for (int i = 0; i < nombre; i++) {
            int j = i + deb;
            int k = i + deb_X;
            X.set(ligne_X, k, get_variation<double>(humidite_, j, ordre_variation));
        }

    }
    void recup_nebulosite(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre, int ordre_variation = 0) {

        for (int i = 0; i < nombre; i++) {
            int j = i + deb;
            int k = i + deb_X;
            X.set(ligne_X, k, get_variation<double>(nebulosite_, j, ordre_variation));
        }

    }
    void recup_pluviometrie(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre, int ordre_variation = 0) {

        for (int i = 0; i < nombre; i++) {
            int j = i + deb;
            int k = i + deb_X;
            X.set(ligne_X, k, get_variation<double>(pluviometrie_, j, ordre_variation));
        }

    }
    void recup_consommation(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre, int ordre_variation = 0) {

        for (int i = 0; i < nombre; i++) {
            int j = i + deb;
            int k = i + deb_X;
            X.set(ligne_X, k, get_variation<double>(consommation_, j, ordre_variation));
        }
    ...
    Cpu::MatrixPair<double> dataset(selection_donnees_t* selec) {
        ...
        recup_consommation(Y, 0, 0, nb_derniere_conso, nb_sample, selec->ordre_variation_conso);
        ...
        {
            recup_consommation(X, c, 0, k, nb_sample, selec->ordre_variation_conso);
            c++;
        }
...
public:
    ...
    void recup_mois(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre) {
        for (int i = 0; i < nombre; i++) {
            int j = i + deb;
            int k = i + deb_X;
            X.set(ligne_X, k, (double)time(j).moi());
    ...
    void recup_jour_semaine(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre) {
        for (int i = 0; i < nombre; i++) {
            ...
            X.set(ligne_X + time(j).jour_semaine(), k, 1./70.);
        }
    }
    void recup_mois_2(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre) {
        for (int i = 0; i < nombre; i++) {
            int j = i + deb;
            int k = i + deb_X;
            for (int w = 0; w < 12; w++)
            ...
            X.set(ligne_X + time(j).moi()-1, k, 1./120.);
        }
    }
    void recup_heure_2(Cpu::Matrix<double>& X, int ligne_X, int deb_X, int deb, int nombre) {
        for (int i = 0; i < nombre; i++) {
            int j = i + deb;
            int k = i + deb_X;
            for (int w = 0; w < 8; w++)
            ...
            X.set(ligne_X + (time(j).heure() / 3), k, 1./240.);
        }
    }
    ...
    Cpu::MatrixPair<double> dataset(selection_donnees_t* selec) {
        ...
        if (selec->mois) {
            recup_mois(X, c, 0, max, N);
            c++;
        }
        if (selec->mois_2) {
            recup_mois_2(X, c, 0, max, N);
            c+=12;
        }
        if (selec->jour) {
