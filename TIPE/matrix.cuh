...
namespace Gpu {
    ...
    void invert_device_float(cublasHandle_t cu_cublasHandle, float* L, float* res, int n)
    {
        ...
        cudaMemcpy(dL, L, szA, cudaMemcpyDeviceToDevice);
        ...
        cudaMemcpy(res, dC, szA, cudaMemcpyDeviceToDevice);
    ...
    void invert_device_double(cublasHandle_t cu_cublasHandle, double* L, double* res, int n)
    {
    ...
    template<> void Matrix<float>::Reverse(const Matrix<float>& A) {
        invert_device_float(ENV_CUBLAS__, A.data().get(), data().get(), A.height());
    }
    template<> void Matrix<double>::Reverse(const Matrix<double>& A) {
        invert_device_double(ENV_CUBLAS__, A.data().get(), data().get(), A.height());
    }
