...
namespace Gpu {
    ...
    public:
        ...
        void __display_errors__(T training_error, T test_error, int iteration) {
            ...
            std::string file_name = "affichage/LearningDisplay.csv";

            std::vector<std::string> vec_line;
            std::cout << "Iteration " << iteration << " : " << training_error << " - " << test_error << "\t";
            vec_line.push_back(std::to_string(iteration));
            vec_line.push_back(std::to_string(training_error));
            vec_line.push_back(std::to_string(test_error));
            std::cout << std::endl;

            File::add_to_csv(file_name, vec_line, true);

        }

        template<template<typename> class Opt> void learning(MatrixPair<T> training, MatrixPair<T> test, HyperParameters<T> hyper_parameters, int nb_epoch) {
            ...
            for (int i = 0; i < nb_epoch; i++)
            {
                ...
                __display_errors__(full_training_error->matrix().get(0, 0), full_test_error->matrix().get(0, 0), i);

            }


        }
        template<template<typename> class Opt> void batch_learning(MatrixPair<T> training, MatrixPair<T> test, HyperParameters<T> hyper_parameters, int nb_epoch) {
            ...
            for (int i = 0; i < nb_epoch; i++)
            {
                ...
                // Display
                compute<T>(full_test_error);
                __display_errors__(training_error->matrix().get(0, 0), full_test_error->matrix().get(0, 0), i);
        ...
        template<template<typename> class Opt> void batch_seq_learning(MatrixPair<T> training, MatrixPair<T> test, HyperParameters<T> hyper_parameters, int nb_epoch, int time_steps, T obj_error = 0, int freq_save = 100) {
            ...
            for (int i = 0; i < nb_epoch; i++)
            {
                ...
                // Display
                __display_errors__(training_error->matrix().get(0, 0), test_error->matrix().get(0, 0), i);

                if (test_error->matrix().get(0, 0) <= obj_error) {
